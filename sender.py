...
def sendDATA(s,receiver_ip,receiver_port,window_size,base,msg,pkg_size,buffer,expected_seq_num) :
    ...
    base = receiveACK(s,packages,receiver_ip,receiver_port,base,buffer,expected_seq_num,pkg_size)   
    return base  

def receiveACK(s,packages,receiver_ip,receiver_port,base,buffer,expected_seq_num,pkg_size) :
    old_base = base
    while(True):
        # receive packet
        ready = select.select([s], [], [], timeout/1000) # this is where we implement the timer.
