...
def sendDATA(s,receiver_ip,receiver_port,window_size,base,msg,pkg_size) :
    ...
    base = receiveACK(s,packages,receiver_ip,receiver_port,base,pkg_size)   
    return base  

def receiveACK(s,packages,receiver_ip,receiver_port,base,pkg_size) :
    old_base = base
    while(True):
        # receive packet
        ready = select.select([s], [], [], timeout/1000) # this is where we implement the timer.
        if ready[0]:
            ...
            if(isNotCorrupted and pkt_header.type == ACK):
                try:  
                    base =pkt_header.seq_num  # ACK received
                    if(pkg_size == 1 and base == 0): # since 1 package messages have special conditions i covered them.
                        return base
                except IndexError: 
                    pass
        else:
